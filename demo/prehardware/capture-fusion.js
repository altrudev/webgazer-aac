...
(function(){
  ...
  const samples=[];
  const state={
    cohort:'unknown', faceMesh:null, ready:false, starting:false, sending:false, timer:null,
    ...
    startupState:'waiting-for-webgazer', startupError:null,
  };
  const EYE={
  ...
  function headFeatures(lms,video,left,right){
    ...
    return {yaw:(nose[0]-eyeMid[0])/inter,pitch:(nose[1]-eyeMid[1])/faceH,roll:Math.atan2(right.center[1]-left.center[1],right.center[0]-left.center[0]),interocularPx:inter};
  }
  ...
  function onResults(results){
    ...
    if(state.model&&fused&&fused.reliability>=.25)state.latestIrisPrediction=C.predict(state.model,featureVector(fused,head,left,right));else state.latestIrisPrediction=null;
    renderLive();
  }
  ...
  function webgazerStartupComplete(){
    try{
      const video=document.getElementById('webgazerVideoFeed');
      return !!(window.webgazer&&typeof window.webgazer.isReady==='function'&&window.webgazer.isReady()&&video&&video.readyState>=2&&video.videoWidth>0&&video.videoHeight>0);
    }catch(_){return false;}
  }
  async function ensure(){
    if(state.ready||state.starting)return;
    if(!webgazerStartupComplete())return;
    state.starting=true;state.startupState='starting-refined-detector';state.startupError=null;renderLive();
    try{
      await new Promise(resolve=>setTimeout(resolve,750));
      if(!webgazerStartupComplete())throw new Error('WebGazer/video readiness changed before refined detector startup');
      ...
      state.faceMesh.onResults(onResults);state.ready=true;state.startupState='ready';state.timer=setInterval(send,100);
  ...
  function sampleAtTarget(target){const e=state.latestEyes,h=state.latestHead;if(!e||!e.fused||e.fused.reliability<.20)return;const features=featureVector(e.fused,h,e.left,e.right);samples.push({features,x:target.x,y:target.y,weight:e.fused.reliability,reliability:e.fused.reliability,webgazer:state.latestWebGazer?{...state.latestWebGazer}:null});if(samples.length>=5)state.model=C.fitLinear(samples);evaluate();renderLive();}
  function evaluate(){if(samples.length<6){state.evaluation=null;return;}const loo=C.leaveOneOut(samples),wgPairs=samples.filter(s=>s.webgazer).map(s=>({px:s.webgazer.x,py:s.webgazer.y,tx:s.x,ty:s.y}));const fusedPairs=[];for(let i=0;i<samples.length;i++){const train=samples.filter((_,j)=>j!==i),m=C.fitLinear(train),ip=C.predict(m,samples[i].features),wg=samples[i].webgazer;if(!ip||!wg)continue;const a=.35+.55*clamp(samples[i].reliability);fusedPairs.push({px:ip.x*a+wg.x*(1-a),py:ip.y*a+wg.y*(1-a),tx:samples[i].x,ty:samples[i].y});}state.evaluation={samples:samples.length,webgazerRmse:C.rmse(wgPairs),irisLooRmse:loo.rmse,fusedLooRmse:C.rmse(fusedPairs)};}
  function renderLive(){
    const q=document.getElementById('captureFusionLive'),e=document.getElementById('captureFusionEval');
    if(q){
      const x=state.latestEyes;
      if(!state.ready&&!x){q.textContent=state.startupState==='error'?`refined detector error: ${state.startupError}`:state.startupState==='starting-refined-detector'?'starting refined detector after WebGazer…':'waiting for WebGazer camera startup';}
      else if(!x)q.textContent='waiting for refined iris capture';
      else q.textContent=`L ${(x.left.reliability*100).toFixed(0)}% · R ${(x.right.reliability*100).toFixed(0)}% · bilateral ${(x.fused.agreement*100).toFixed(0)}% · capture ${(x.fused.reliability*100).toFixed(0)}%${x.fused.reliability<.35?' · PAUSE QUALITY':''}`;
    }
    if(e){const v=state.evaluation;e.textContent=!v?`${samples.length} calibration samples collected`:`${v.samples} samples · WebGazer ${v.webgazerRmse==null?'—':v.webgazerRmse.toFixed(1)+' px'} · iris LOO ${v.irisLooRmse==null?'—':v.irisLooRmse.toFixed(1)+' px'} · fused LOO ${v.fusedLooRmse==null?'—':v.fusedLooRmse.toFixed(1)+' px'}`;}
  }
  function mount(){const section=document.createElement('section');section.id='captureFusionPanel';section.className='panel';section.innerHTML=`<div class="panelHead"><div><p class="eyebrow">EXPERIMENTAL CAPTURE FUSION</p><h2>Iris + head pose + per-eye reliability</h2></div><label>Validation cohort <select id="eyewearCohort"><option value="unknown">Unknown</option><option value="none">No eyewear</option><option value="glasses">Glasses</option><option value="contacts">Contacts</option></select></label></div><p class="hint">Cohort is user-selected evidence only; eyewear is never inferred. Refined MediaPipe starts only after WebGazer and the camera are fully initialized to avoid WASM-runtime collision.</p><div class="cameraComparisonGrid"><article><span>Live capture reliability</span><strong id="captureFusionLive">waiting for WebGazer camera startup</strong></article><article><span>Predictor comparison</span><strong id="captureFusionEval">0 calibration samples collected</strong></article></div><div class="controls"><button id="downloadCaptureFusion" type="button">Download fusion evidence</button></div>`;const evidence=document.querySelector('.evidencePanel');if(evidence&&evidence.parentNode)evidence.parentNode.insertBefore(section,evidence);else document.querySelector('main').appendChild(section);section.querySelector('#eyewearCohort').addEventListener('change',ev=>state.cohort=ev.target.value);section.querySelector('#downloadCaptureFusion').addEventListener('click',download);document.addEventListener('click',ev=>{const b=ev.target.closest&&ev.target.closest('.calibrationPoint');if(!b)return;const r=b.getBoundingClientRect();sampleAtTarget({x:r.left+r.width/2,y:r.top+r.height/2});});}
  function evidence(){return {schema:'webgazer-aac/capture-fusion-experiment/0.2',exportedAt:new Date().toISOString(),experimental:true,canonicalRuntimeModified:false,eyewearCohort:state.cohort,rawEyeMaterialPersisted:false,rawLandmarksPersisted:false,startup:{state:state.startupState,error:state.startupError,deferredUntilWebGazerReady:true},frames:state.frames,missing:state.missing,latest:{eyes:state.latestEyes,head:state.latestHead,eyePatchQuality:state.latestEyePatch},calibrationSamples:samples.length,evaluation:state.evaluation};}
  ...
  mount();
  const patchPoll=setInterval(()=>{if(patchGaze())clearInterval(patchPoll);},100);
  const startupPoll=setInterval(()=>{if(state.ready){clearInterval(startupPoll);return;}if(webgazerStartupComplete())ensure();},250);
  window.webgazerCaptureFusion={getLive:()=>({available:!!state.latestEyes,reliability:state.latestEyes&&state.latestEyes.fused?state.latestEyes.fused.reliability:0,leftReliability:state.latestEyes?state.latestEyes.left.reliability:0,rightReliability:state.latestEyes?state.latestEyes.right.reliability:0,prediction:state.latestIrisPrediction,startupState:state.startupState}),getEvidence:evidence,getEvaluation:()=>state.evaluation};
})();